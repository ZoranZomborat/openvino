...
std::vector<std::string> disabledTestPatterns() {
    ...
    static const auto skipRegistry = []() {
        ...
        // [Tracking number: E#133976]
        _skipRegistry.addPatterns((backendName.isIMD()),
                "MemoryLSTMCellTest failing with unsupported platform", {
        ...
        // [Tracking number E#124966]
        _skipRegistry.addPatterns(devices.has6010(),
                "Accuracy issues with asymetric Interpolate with size 1, 2, 8, 16", {
                ".*smoke_Interpolate_Linear_Asymmetric_.*1.2.8.16.*",
        });

        // [Tracking number E#124966]
        _skipRegistry.addPatterns(devices.has6010(),
                "Hangs in IMD backend NPU6", {
                ".*smoke_precommit_GRUSequence_Split.*1.1.10.*",
                ".*smoke_precommit_GRUSequence_Tiling_BI.*100.1000.10.*",
                ".*smoke_GroupConvolution3D_ExplicitPadding.*1.4.10.10.10.*",
        });
